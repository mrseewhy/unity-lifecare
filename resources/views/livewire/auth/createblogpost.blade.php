        ...
        <div>
            ...
            @enderror
                ...
                <div>
                    ...
                    <input type="file" wire:model="featured_image" class="w-full p-2 border rounded">
                    @if ($featured_image)
                        <img src="{{ $featured_image->temporaryUrl() }}" class="mt-2 h-32 object-cover rounded">
                    @endif
                    @error('featured_image') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <button type="submit" class="bg-purple-700 text-white px-4 py-2 rounded w-full block mt-12 mb-8">Save Post</button>
            </form>
        </div>
    </div>
</div>
